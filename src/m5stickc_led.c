...
static const char * TAG = "m5stickc_led";

static bool current_state = M5STICKC_LED_DEFAULT_STATE;
...
esp_err_t M5StickCLedInit()
{
    ...
    io_conf.pin_bit_mask = ((1ULL << M5STICKC_LED_GPIO)); // bit mask of the pins that you want to set, e.g.GPIO10
...
esp_err_t M5StickCLedSet(bool state)
{
    ...
    current_state = state;
    e = gpio_set_level(M5STICKC_LED_GPIO, current_state);
    if (e != ESP_OK)
    {
        return ESP_FAIL;
    }
...
esp_err_t M5StickCLedToggle(void)
{
    ...
    e = M5StickCLedSet(1 - current_state);
    if (e != ESP_OK)
    {
        return ESP_FAIL;
    }
